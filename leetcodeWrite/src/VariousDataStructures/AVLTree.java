...
public class AVLTree<E> {
	 ...
	 * B的左子树变为p的右子树
	 * （先调整A和BL的关系，再调整B的父节点为A的父节点，最后调整A和B的关系）
	 * 比如：     	  A(-2)                   B(1)
	 *              \        左旋转       		  /   \
	 *             B(0)     ---->       A(0)   BR(0)       
	 *             /   \                   \    
	...
	private void rotateLeft(Entry<E> p) {
		...
		if(p!=null) {
			Entry<E> r = p.right;
			p.right =r.left;//把p右子树的左节点嫁接到p的右节点，r(B)是p(A)的右子树,p(A)的右子树(B)变为r(B)的左子树(BL)
	 ...
	 * 2.自下向上进行回溯，回溯做两个事情：
	 * (1)修改祖先节点的平衡因子，当插入 一个节点时，只有根节点到插入节点的路径中的节点的平衡因子会被改变，
	 * 而且改变的原则是当插入节点在某节点(称为A)的左子树中时，A的平衡因子(称为BF)为BF+1,当插入节点在A的右子树中时A的BF-1，
	 * 而判断插入节点在左子树中还是右子树中只要简单的比较它与A的大小
	 * (2)在改变祖先节点的平衡因子的同时，找到最近一个平衡因子大于2或小于-2的节点，
	 * 从这个节点开始调整最小不平衡树进行旋转调整，调整方法为fixAfterInsertion。
	...
	/**
	 * 调整的方法：
	 * 1.当最小不平衡子树的根(R)为2时，即左子树高于右子树：
	 * 如果R的左子树的根节点的BF为1时，做右旋；（就是LL）
	 * 如果R的左子树的根节点的BF为-1时，先左旋然后再右旋(就是LR)
	 * 
	 * 2.R为-2时，即右子树高于左子树：
	 * 如果R的右子树的根节点的BF为1时，先右旋后左旋(RL)
	 * 如果R的右子树的根节点的BF为-1时，做左旋(RR)
	 * @param parent
	 */
	...
	/**
	 * 	 * 做左平衡处理
	 * 平衡因子的调整如图： 情况1，2，3都是左子树的右子树深，分情况是因为调整后t l位置的balance不一样
	 * 
	 *         t                       rd
	 *       /   \                   /    \
	 *      l    tr   左旋后右旋                      l       t
	 ...
	 *         12  15   24
	 *         
	 * 例如18，左右子树都不为空，先找到直接后继22。18位置数据变为22，p18变为s22
	 * replacement=24,p.parent.left/right = 24
	 * 
	 * @param e
	 */
	private void deleteEntry(Entry<E> p) {
		...
		size --;
		Entry<E> nex = p.right;
		System.out.println("p.r="+nex.element);
		//如果p左右子树都不为空，找到其直接后继，替换p，之后p指向s，删除p其实是删除s
		//所有的删除左右子树不为空的节点都可以调整为删除左右子树有其一不为空，或都为空的情况。
		if (p.left != null && p.right != null) {
			 Entry<E> s = successor(p);
			 System.out.println("删除中s=="+s.element);
			 p.element = s.element;//这里要删除的那个节点的left right已经给了后继节点，就是把后继节点的数值替换p位置数值
			 p = s;//这里p=s p就是s节点了，后面操作就是来删除s节点的影响
		}
		System.out.println("p.r1="+nex.parent.element);
		System.out.println("p.r2="+nex.parent.right.element);
		System.out.println("p.r3="+nex.parent.left.element);
		
		System.out.println("删除中p=="+p.element);
		if(p.left!=null)
			System.out.println("删除中pl=="+p.left.element);
		if(p.right!=null)
			System.out.println("删除中pr=="+p.right.element);
		
		Entry<E> replacement = (p.left != null ? p.left : p.right);
        if (replacement != null) {		//如果其左右子树有其一不为空
        	System.out.println("删除中replacement=="+replacement.element);
    		...
    		System.out.println("删除中pF=="+p.element);
    		if(p.left!=null)
    			System.out.println("删除中pFl=="+p.left.element);
    		if(p.right!=null)
    			System.out.println("删除中pFr=="+p.right.element);
            if (p.parent != null) {
                if (p == p.parent.left)
                    p.parent.left = null;
	...
	/**
	 * 返回以中序遍历方式遍历树时，t的直接后继，就是和t值最近的那个比t大的节点
	 */
	...
	static class Entry<E>{
		...
		Entry<E> left;
		Entry<E> right;
		int balance = EH;//平衡因子，只能为1/0/-1否则就需要平衡
